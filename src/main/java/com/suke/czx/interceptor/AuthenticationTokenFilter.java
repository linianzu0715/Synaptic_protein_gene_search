...
public class AuthenticationTokenFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if(StrUtil.isNotBlank(token) && !StrUtil.equals(token,"null")){
            redisTemplate.opsForValue().set("token","6cd0336372d7480a9d67c825ced7ee3d",60*60*24,TimeUnit.SECONDS);
            redisTemplate.opsForValue().set(token,"test",Constant.TOKEN_EXPIRE, TimeUnit.SECONDS);
            Object userId = redisTemplate.opsForValue().get(token);
            if(ObjectUtil.isNull(userId)){
                writer(response,"无效token");
                return;
            }
            UserDetails userDetails = customUserDetailsService.loadUserByUserId((1L));
